...
int main()
{
    ...
    for (int i = 0; i < ops+1; i++){
        ...
        if (temp->sign){
            ...
            sprintf(table[table_i], "%d T%d %c _ -", table_i+1, table_i+1, (char)temp->val);
            printf("%s\n",table[table_i]);
            temp->val = table_i + 1;
            table_i++;
            temp->sign = 0;
        ...
        sprintf(table[table_i], "%d T%d %s %s %c", table_i+1, table_i+1, prev_str, next_str, (char)temp->val);
        printf("%s\n",table[table_i]);
    ...
    if (is_assignment){
        if (table_i == 0) 
            printf("%d %c %c _ = \n", table_i+1, buff[0],  (char)Head->next->next->next->val);
        else 
            printf("%d %c T%d _ = \n", table_i+1, buff[0], table_i-1);
    }
}
